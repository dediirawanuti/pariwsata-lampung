...
{
    $db      = db_connect();
    $query = $db->table('galery')->where('tipe', 'brand')->get();
    return $query->getResultArray();
}

function get_rating($maps_id)
{
    $db      = db_connect();
    $query = $db->table('rating')
        ->selectAvg('rating', 'rating')
...
function breadcrumbs($separator = '&nbsp &raquo; &nbsp', $home = 'Dashboard')
{
    ...
    $breadcrumbs = array("<a href=\"$dash\" style=\"color: #696cff;\"><span>" . ucfirst($home) . "</span></a>");
    ...
    foreach ($path as $x => $crumb) {
        ...
        if ($x != $last) {
            $breadcrumbs[] = "<a href=\"$base$crumbs$crumb\"><span class=\"text-muted fw-light\">" . ucfirst($title) . "</span></a>";
            $crumbs .= $crumb . '/';
        }
